...
const Calendar = (props) => {
    ...
    const getUserInfo = async () => {
        const userLoggedInfo = await getDoc(doc(db, 'users', the_user));
    ...
    const deleteFromCalendar = async () => {
        ...
        console.log("estou na function de delete")
        console.log(selectedToDelete)
       await deleteDoc(doc(db, `users/${the_user}/calendar`, selectedToDelete));
    ...
    const calendarWrite = async () => {
        ...
        const calendarRef = collection(db, `users/${the_user}/calendar`);
    ...
    console.log("ids das entradas nos calendários"+entriesTracksDocId)
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < daysInMonth; i++) {
            ...
            entriesTracksDay.forEach(element => {
                if(element == i){
                    daysTrue.splice(i, 1, 1)
                }
            })
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            console.log("toma la esta"+entriesTracksDocId[contador]);
            days.push(<td className='col-span-1 px-3 my-3 border-l-2 border-preto/50 hover:border-preto text-xl text-preto/50 hover:text-preto h-[13rem] flex flex-col' key={i }><div className='flex h-[30%]'>{i} {daysTrue[i] == 1 && entriesTracks[contador]}</div> <div className='group flex w-full justify-center items-center pt-2'>{daysTrue[i] == 1 && <><img className='w-[7rem]' src={entriesTracksImage[contador]} /><button className='text-cinza hidden group-hover:flex justify-center absolute w-[7rem] h-[7rem] items-center bg-preto/70  cursor-pointer' onClick={() => {console.log("delete"); setSelectedToDelete(entriesTracksDocId[contador]); modalDeleteFunction() }}><RxCross1 className=' text-2xl'></RxCross1></button> 
            ...
            <div className='bg-preto w-[30vw] h-auto shadow-2xl flex flex-col justify-center text-center items-center rounded-xl p-8'>
                    ...
                    <button type='button' className='mr-2 py-2 px-5  w-[5rem] rounded-full bg-amarelo text-xl text-preto outline-none ' onClick={() => { modalDeleteFunction(); deleteFromCalendar() }}>Yes</button>
            ...
            <section className='px-10'>
                ...
                <table className='w-full'>
                    <thead className='w-full'>
                        <tr className='grid grid-cols-7 w-full text-center text-preto text-4xl'>
